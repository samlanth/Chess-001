...
namespace chess
{
    ...
    template <typename PieceID, typename uint8_t _BoardSize, typename TYPE_PARAM, int PARAM_NBIT> class BaseGame;
    ...
    public:
        ...
        virtual void play(bool verbose = false);
    ...
    // play()
    template <typename PieceID, typename uint8_t _BoardSize, typename TYPE_PARAM, int PARAM_NBIT>
    inline void BaseGame<PieceID, _BoardSize, TYPE_PARAM, PARAM_NBIT>::play(bool verbose)
    {
        size_t move_idx;
        std::vector<_Move>  m;
        ...
        while (true)
        {
            ...
            if ((board.is_final(m))||(board.get_histo_size() >= _max_game_ply))
                break;
